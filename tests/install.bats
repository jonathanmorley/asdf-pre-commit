...
install() {
  ...
  export ASDF_INSTALL_PATH="${TMPDIR:-/tmp}/asdf-test-${version}"
...
@test "install and verify all major versions" {
  for version in $(get_versions_to_test); do
    echo "# Testing pre-commit $version" >&3
    install "$version"

    [ -x "$ASDF_INSTALL_PATH/bin/pre-commit" ]
    [ "$("$ASDF_INSTALL_PATH/bin/pre-commit" --version)" = "$version" ]

    rm -rf "$ASDF_INSTALL_PATH"
  done
}

@test "pre-commit binary works correctly" {
